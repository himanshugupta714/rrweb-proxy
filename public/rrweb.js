var rrweb = (function (exports) {
  ...
  function initInputObserver({
    ...
    function cbWithDedup(target, v) {
      ...
      if (
        ...
        const id = mirror.getId(target);
        inputCb(
          __spreadProps$4(__spreadValues$4({}, v), {
            id,
          })
        );
      }
  ...
  class ShadowDomManager {
    ...
    addShadowRoot(shadowRoot, doc) {
      ...
      initMutationObserver(
        __spreadProps$3(__spreadValues$3({}, this.bypassOptions), {
          doc,
  ...
  class Replayer {
          ...
          if ("commands" in data) {
            ...
            if (status.isUnchanged === false)
              this.canvasEventMap.set(
                event,
                __spreadProps(__spreadValues({}, data), { commands })
              );
          } else {
            const args = yield Promise.all(
            ...
            if (status.isUnchanged === false)
              this.canvasEventMap.set(
                event,
                __spreadProps(__spreadValues({}, data), { args })
              );
          }
        }
      });
    ...
    applyMutation(d, isSync) {
      ...
      const legacy_missingNodeMap = __spreadValues(
        {},
        this.legacy_missingNodeRetryMap
      );
      const queue = [];
